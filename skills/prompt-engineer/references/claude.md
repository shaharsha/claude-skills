...
Opus 4.8 offers **fast mode** (research preview on the Claude API): set `speed: "fast"` for up to ~2.5× higher output tokens/sec from the *same model* at premium pricing. It changes throughput, not the model or its quality — reach for it on latency-sensitive interactive surfaces, not to save cost.
