...
| Default state | Opus 4.8: thinking **OFF** unless `thinking:{type:"adaptive"}` is set (Sonnet 5: on by default; Fable 5: always on); `effort` defaults to `high`, `xhigh` for coding/agentic | Default effort per-model: GPT-5.6 std-mode `[unverified]` (pro `medium`), GPT-5.4/Mini/Nano `none` | Default `thinking_level` per-model: 3.1 Pro `high`, 3.6 Flash `medium`, 3.5 Flash-Lite `minimal` |
