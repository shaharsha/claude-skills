...
scripts/ccsend --ping                            # round-trip: proves DELIVERY, not just a process
...
- **A fresh lease is not proof of delivery.** `--self` shows a heartbeat naming a live pid. It does *not* show a Monitor is attached to that watcher's stdout, and a **pid can be reused** inside the 10-second freshness window. Use **`ccsend --ping`** for the round trip that actually proves the path; arrival is proof, non-arrival is ambiguous, and the two checks together separate "no watcher" from "watcher running, nobody consuming it".
