...
CCPEERS="$HOME/.claude/skills/working-with-other-sessions/scripts/ccpeers"
...
# ---------------------------------------------------------------- lanes
echo "## LIVE SESSIONS"
# 2026-08-12: this section used to count '📬 armed' from `ccsend --list`. Arming was retired
# with the spool that day and NOTHING arms any more -- so the count became a permanent 0 and
# the script printed "ZERO armed sessions is a finding... everyone is genuinely gone" on a
# fleet of eight healthy lanes. A census that manufactures an emergency at every takeover.
# Exactly the class this file's own header warns about, in the file that warns about it.
if [ -x "$CCPEERS" ]; then
  ROSTER=$("$CCPEERS" 2>/dev/null)
  # Count rows, not a status word: reachability now lives in the flag column, and any
  # word we grep for is a word that can be renamed out from under us again.
  N_ROWS=$(printf '%s\n' "$ROSTER" | sed -n '3,$p' | grep -cE '^[a-z]')
  N_BAD=$(printf '%s\n' "$ROSTER" | grep -cE 'UNREACHABLE|RESTART')
  echo "  registered sessions: ${N_ROWS:-0}   flagged unreachable/stale: ${N_BAD:-0}"
  printf '%s\n' "$ROSTER" | sed 's/^/  /'
  if [ "${N_ROWS:-0}" -eq 0 ]; then
    echo "  ^ ZERO rows is a finding, not a quiet sprint. Either every session is genuinely" >&2
    echo "    gone, or ccpeers is broken and you are about to conclude the first." >&2
  fi
else
  echo "  ccpeers not found at $CCPEERS — cannot see any lane. This is a BROKEN INSTRUMENT," >&2
  echo "  not an empty roster." >&2
fi
echo
echo "  NOTE: this is REACHABILITY. It does not say what any lane is DOING — that is ccstatus,"
echo "  which classifies on the last event's TYPE (IN-FLIGHT / SPOKE / QUEUED). A lane that is"
echo "  reachable and stalled looks identical here to one that is reachable and working."
echo
