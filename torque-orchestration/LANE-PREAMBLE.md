...
```
DO NOT   hand-roll a fan-out loop in zsh
USE      ~/.claude/torque-orchestration/ccbroadcast.sh --file <path>
         bash shebang · array expansion · per-send status captured inside the invocation
         · and it VERIFIES each message in the RECIPIENT'S OWN inbox, exiting 1 if any is
           missing, because a send loop that ticks per iteration proves nothing arrived
```

**`ccsend` exit 0 means ARMED AND SPOOLED. It does not mean delivered, and it never meant received.**
...
their `echo`'s. **Read the status in the SAME invocation as the command, unpiped, with nothing
between.** A peer re-measured and overturned it — six shapes, only the piped and the separated ones
returned `0`.
...
**Check your OWN inbox registration periodically — `persistent: true` is not enough.** A dozen lanes
were listed `unarmed` while believing they were listening, some with a live `ccarm` process. `ccsend`
**refuses** an unarmed target (measured unpiped, with controls: dead → exit 1, bogus → exit 1, armed →
exit 0), so nothing you are sent is silently dropped — **but the sender learns and you never do.**
Having received a message recently is no evidence: the monitor delivers, then exits.

```bash
~/.claude/skills/reading-session-transcripts/scripts/ccsend --list     # find your own id
```
...
screen is the hook's own test summary. **Never pipe a push** — a pipeline returns its last command's
status, so `$?` reads 0 whether the push landed or the hook refused it, measured at every tail window.
...
`ccsend --list` and tell them. Unpushed work is invisible to every git question they can ask, and each
of those questions answers *truthfully*. Lane-to-lane at the moment of exposure beats routing through
the coordinator.
...
```
IN PLAN MODE   reads and ccsend only
